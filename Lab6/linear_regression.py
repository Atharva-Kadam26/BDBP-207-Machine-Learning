...
def linear_regression(X,y,alpha,iterations,test_s):
    ...
    #   y_df = data.iloc[:,y_column]
    #   X = X_df.values.tolist()
    #   y = y_df.values.tolist()
    #   return X,y

    def split_data(X,y):
      X_train,X_test,y_train,y_test = train_test_split(X,y,test_size=test_s,random_state=999)
    ...
    def cost_func(y_train,predictions):
      total_error=0
    ...
    def gradient_descent(X_train_scale_int,thetas,y_train,alpha,iterations,tolerance=1e-3):
    ...
    def execute():
      ...
      total_error=cost_func(y_train,predictions)
      thetas=gradient_descent(X_train_scale_int,thetas,y_train,alpha,iterations)
