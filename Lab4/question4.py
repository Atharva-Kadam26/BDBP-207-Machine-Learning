...
def plotting_the_data(y_test_l,y_test_pred):
    plt.scatter(y_test_l,y_test_pred,label="Normal Equation",color="green")
    min_y_2=min(y_test_l)
    max_y_2=max(y_test_l)
    plt.plot([min_y_2,max_y_2],[min_y_2,max_y_2],'k--',label="Ideal Fit")
...
def main():
    ...
    plotting_the_data(y_test_l,y_test_pred)
...
def split_data(X,y):
    X_train,X_test,y_train,y_test_2=train_test_split(X,y,test_size=0.3,random_state=999)
    return X_train,X_test,y_train,y_test_2
...
def predicting_y(model,X_test_scale):
    y_pred_2=model.predict(X_test_scale)
    return y_pred_2

def evaluating(y_test_2,y_pred_2):
    r2=r2_score(y_test_2,y_pred_2)
    return r2

def plotting_the_data(y_test_2,y_pred_2):
    plt.scatter(y_test_2,y_pred_2,label="Scikit Learn Model",color="blue")
    min_y_2=min(y_test_2)
    max_y_2=max(y_test_2)
    plt.plot([min_y_2,max_y_2],[min_y_2,max_y_2],'k--',label="Ideal Fit")
    plt.xlabel("Actual Value")
    plt.ylabel("Predicted Value")
    plt.legend()
...
def main():
    X,y=load_data("Admission_Predict.csv")
    X_train,X_test,y_train,y_test_2=split_data(X,y)
    X_train_scale,X_test_scale=standardize_data(X_train,X_test)
    model=linear_regression(X_train_scale,y_train)
    y_pred_2=predicting_y(model,X_test_scale)
    r2=evaluating(y_test_2,y_pred_2)
    print("R2 score by Scikit-Learn Model is:",r2)
    plotting_the_data(y_test_2,y_pred_2)
...
def plotting_the_data(y_test,y_test_pred):
    plt.scatter(y_test,y_test_pred,label="Using Gradient Descent",color="red")
    min_y_3=min(y_test)
    max_y_3=max(y_test)
    plt.plot([min_y_3,max_y_3],[min_y_3,max_y_3],'k--',label="Ideal Fit")
    plt.xlabel("Actual Value")
    plt.ylabel("Predicted Value")
    plt.legend()
    return plt.show()
...
def main():
    ...
    return plotting_the_data(y_test,y_test_pred)
